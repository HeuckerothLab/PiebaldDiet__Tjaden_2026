#===============================================================================
# Script Name:    volcano.R
# Purpose:        Function for making ggplot2-based volcano plot
# Author:         beigelk
# GitHub:         https://github.com/beigelk
# Source:         https://github.com/beigelk/volcano-fig
#===============================================================================
# Notes:          package requirements: tidyverse, ggplot2, ggrepel
#                 R files: utils.R
#===============================================================================
...
#' change are colored/labeled with \code{condition_up}; genes with a p-value
#' below \code{padj_thresh} and a negative log2 fold change are
#' colored/labeled with \code{condition_down}; all other genes are marked as
...
#' @param plot_name Character. Base name used (together with
#'   \code{plot_version}) to construct the output PDF/PNG filenames, in the
#'   form \code{<plot_name>_<plot_version>.pdf} / \code{.png}.
...
#'   (\code{-log10(pval_col)}).
#' @param condition_down Character. Value used to label/color genes with a
...
#'   annotation.
#' @param condition_up Character. Value used to label/color genes with a
...
#'   \code{condition_down} entry in the color scale, and the down-regulated
#'   annotation text.
#' @param color_up Character. Color (name or hex code) used for the
#'   up-regulated group: point fill for the outlined top genes, the
#'   \code{condition_up} entry in the color scale, and the up-regulated
#'   annotation text.
...
#'   label_col      = "gene",
#'   pval_col       = "padj",
#'   condition_down = "Condition A",
#'   condition_up   = "Condition B",
...
plot_volcano = function(
    ...
    vol_data = deg_file %>%
        ...
        mutate(
            color = case_when(
                !!sym(log2FC_col) > 0 & !!sym(pval_col) < padj_thresh ~ condition_up,
                !!sym(log2FC_col) < 0 & !!sym(pval_col) < padj_thresh ~ condition_down,
                .default = 'n.s.'
            )
        )
    ...
    ggvol = vol_data %>%
        ...
        scale_color_manual(
            breaks = c(condition_down, condition_up),
            values = rlang::list2(
                !!condition_down := color_down,
                'n.s.' = ns_color,
                !!condition_up := color_up
            )
        ) +
        
        # Another option for trying to control text positions
        # geom_text_repel() +
  ...
  plot_pdf_and_png(
      plot = ggvol,
      filename = paste0(plot_name, '_', plot_version),
      width = plot_width,
      height = plot_height
  )
